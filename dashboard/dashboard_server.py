...
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .band-card.b30::before{background:var(--band30)}
  ...
  .band-card.b30 .band-name{color:var(--band30)}
  ...
  /* ── Alert banner stack (storm + radio blackout can both show) ── */
  .alert-stack { position:sticky; top:0; z-index:200; display:flex; flex-direction:column; }

  /* ── Storm Alert Banner ── */
  .storm-banner { display:none; }
  ...
  /* ── Radio Blackout Alert Banner (X-ray flare -> NOAA R-scale) ── */
  .blackout-banner { display:none; }
  .blackout-banner.visible { display:block; }
  .blackout-inner { display:flex; align-items:center; gap:16px; padding:12px 24px; flex-wrap:wrap; }
  .blackout-banner.CAUTION  .blackout-inner { background:#00303d; border-bottom:3px solid #00d4ff; }
  .blackout-banner.WARNING  .blackout-inner { background:#241b40; border-bottom:3px solid #a78bfa; }
  .blackout-banner.CRITICAL .blackout-inner { background:#2d0033; border-bottom:3px solid #ec4899; animation:flash-blackout 0.8s infinite; }
  @keyframes flash-blackout { 0%,100%{background:#2d0033} 50%{background:#560066} }
  .blackout-icon { font-size:26px; flex-shrink:0; }
  .blackout-text { flex:1; }
  .blackout-title { font-family:'Orbitron',monospace; font-size:13px; font-weight:700; letter-spacing:2px; margin-bottom:4px; }
  .blackout-banner.CAUTION  .blackout-title { color:#00d4ff; }
  .blackout-banner.WARNING  .blackout-title { color:#a78bfa; }
  .blackout-banner.CRITICAL .blackout-title { color:#ec4899; text-shadow:0 0 10px rgba(236,72,153,0.8); }
  .blackout-detail { font-family:'Share Tech Mono',monospace; font-size:11px; color:rgba(255,255,255,0.85); line-height:1.6; }
  .blackout-dismiss { font-family:'Share Tech Mono',monospace; font-size:10px; letter-spacing:1px;
    padding:6px 14px; background:transparent; cursor:pointer; flex-shrink:0; transition:all 0.2s; border-radius:2px; }
  .blackout-banner.CAUTION  .blackout-dismiss { border:1px solid #00d4ff; color:#00d4ff; }
  .blackout-banner.WARNING  .blackout-dismiss { border:1px solid #a78bfa; color:#a78bfa; }
  .blackout-banner.CRITICAL .blackout-dismiss { border:1px solid #ec4899; color:#ec4899; }
  .blackout-dismiss:hover { background:rgba(255,255,255,0.1); }
  .blackout-status-pill { font-family:'Share Tech Mono',monospace; font-size:10px;
    padding:3px 10px; border-radius:2px; letter-spacing:1px; display:inline-block; margin-top:4px; }
  .blackout-status-NONE     { color:var(--good); border:1px solid var(--good); background:rgba(57,255,20,0.08); }
  .blackout-status-CAUTION  { color:#00d4ff;     border:1px solid #00d4ff;     background:rgba(0,212,255,0.12); }
  .blackout-status-WARNING  { color:#a78bfa;     border:1px solid #a78bfa;     background:rgba(167,139,250,0.12); }
  .blackout-status-CRITICAL { color:#ec4899;     border:1px solid #ec4899;     background:rgba(236,72,153,0.15); animation:flash-blackout 0.8s infinite; }
</style>
</head>
<body>

<!-- Alert banner stack: storm (tower/weather) and radio blackout (propagation) are
     independent and can both be visible at the same time -->
<div class="alert-stack">
  ...
  <div class="blackout-banner" id="blackoutBanner">
    <div class="blackout-inner">
      <div class="blackout-icon" id="blackoutIcon">!</div>
      <div class="blackout-text">
        <div class="blackout-title" id="blackoutTitle">RADIO BLACKOUT</div>
        <div class="blackout-detail" id="blackoutDetail">Checking solar data...</div>
      </div>
      <button class="blackout-dismiss" onclick="dismissBlackout()">X DISMISS</button>
    </div>
  </div>
</div>
...
<div class="solar-bar">
  ...
  <div class="solar-item">
    <div class="solar-label">RADIO BLACKOUT</div>
    <div id="blackoutPill" class="blackout-status-pill blackout-status-NONE">ALL CLEAR</div>
    <div class="solar-sub" id="blackoutSub">X-ray flare class</div>
  </div>
  <div class="solar-item">
...
<div style="background:var(--panel);border-bottom:1px solid var(--border);padding:10px 24px;display:flex;align-items:center;gap:20px;flex-wrap:wrap">
  ...
  <div style="margin-left:auto;display:flex;flex-direction:column;gap:5px;flex-shrink:0">
  ...
  <!-- DX Calendar / DX Expeditions -->
  <!-- Switched away from DXNews.com's embedded widget 2026-08-11 (had gone stale -- serving
       6-month-old data). DX-World.net was tried as an iframe embed next but confirmed blocked
       (broken frame) same day. Rather than keep hunting for a third embeddable provider --
       DX calendar/expedition widgets have now failed two different ways in one morning --
       settled on plain link-out buttons, same treatment DXSummit/DXWatch already get in the
       DX Cluster Spots panel below, since those also refuse to embed. NG3K's ADXO page is a
       real destination here too (permission requested from NG3K 2026-08-11 for future
       *automated* retrieval -- this is just a manual link, no automated access, so no
       permission concern). Genuine inline DX-expedition data is expected to come later via
       the DXMon project's backend JSON API once that exists, per the Desktop Instrument
       Series' "DXMon -- DX Monitor Scoping & Architecture" note. -->
  <div style="flex-shrink:0;display:flex;flex-direction:column;gap:6px;margin-left:20px">
    <div style="font-family:'Orbitron',monospace;font-size:9px;letter-spacing:3px;color:var(--accent2);white-space:nowrap">DX EXPEDITIONS</div>
    <div style="border:1px solid var(--border);background:var(--bg);padding:10px;min-width:320px;display:flex;flex-direction:column;gap:8px;justify-content:center">
      <a href="https://www.dx-world.net/" target="_blank" style="font-family:'Share Tech Mono',monospace;font-size:11px;letter-spacing:1px;padding:7px 18px;border:1px solid var(--accent2);color:var(--accent2);text-decoration:none;transition:all 0.2s;text-align:center" onmouseover="this.style.background='var(--accent2)';this.style.color='var(--bg)'" onmouseout="this.style.background='transparent';this.style.color='var(--accent2)'">&#9654; OPEN DX-WORLD.NET IN NEW TAB</a>
      <a href="https://www.ng3k.com/misc/adxo.html" target="_blank" style="font-family:'Share Tech Mono',monospace;font-size:11px;letter-spacing:1px;padding:7px 18px;border:1px solid var(--accent2);color:var(--accent2);text-decoration:none;transition:all 0.2s;text-align:center" onmouseover="this.style.background='var(--accent2)';this.style.color='var(--bg)'" onmouseout="this.style.background='transparent';this.style.color='var(--accent2)'">&#9654; OPEN NG3K ADXO IN NEW TAB</a>
    </div>
  </div>
</div>
...
<div class="main">
  ...
  <div class="panel band-panel">
    ...
    <p class="data-note">Solar indices from NOAA SWPC. HamQSL widget updated every 3 hours. 80m/40m/30m rated for D-layer absorption during day.</p>
  </div>
  ...
  <div class="panel map-panel">
    ...
    <div style="display:flex;gap:6px;margin-bottom:8px;align-items:center">
      ...
      <a href="https://hamdash.com/" target="_blank"
         style="font-family:'Share Tech Mono',monospace;font-size:10px;letter-spacing:1px;padding:5px 14px;border:1px solid var(--accent);color:var(--accent);text-decoration:none;transition:all 0.2s"
      ...
      <a href="https://dxlook.com/" target="_blank"
         style="font-family:'Share Tech Mono',monospace;font-size:10px;letter-spacing:1px;padding:5px 14px;border:1px solid var(--accent);color:var(--accent);text-decoration:none;transition:all 0.2s"
         onmouseover="this.style.background='var(--accent)';this.style.color='var(--bg)'"
         onmouseout="this.style.background='transparent';this.style.color='var(--accent)'">
        &#9654; DXLOOK (NEW TAB)
      </a>
      <a href="https://openhamclock.com/" target="_blank"
         style="font-family:'Share Tech Mono',monospace;font-size:10px;letter-spacing:1px;padding:5px 14px;border:1px solid var(--accent);color:var(--accent);text-decoration:none;transition:all 0.2s"
         onmouseover="this.style.background='var(--accent)';this.style.color='var(--bg)'"
         onmouseout="this.style.background='transparent';this.style.color='var(--accent)'">
        &#9654; OPENHAMCLOCK (NEW TAB)
      </a>
    </div>
  ...
  <div class="panel links-panel">
    ...
    <a class="cluster-link" href="https://hamdash.com/" target="_blank">&#9654; HAMDASH</a>
    <a class="cluster-link" href="https://dxlook.com/" target="_blank">&#9654; DXLOOK</a>
    <a class="cluster-link" href="https://openhamclock.com/" target="_blank">&#9654; OPENHAMCLOCK</a>
    ...
    <a class="cluster-link accent2" href="https://www.dx-world.net/" target="_blank">&#9654; DX-WORLD.NET</a>
    <a class="cluster-link accent2" href="https://www.ng3k.com/misc/adxo.html" target="_blank">&#9654; NG3K ADXO</a>
  </div>
...
// Band config
const bandColors = {
  '80m':'var(--band80)','40m':'var(--band40)','30m':'var(--band30)','20m':'var(--band20)','17m':'var(--band17)',
  '15m':'var(--band15)','12m':'var(--band12)','10m':'var(--band10)','6m':'var(--band6)'
};
const bandClass = {'80m':'b80','40m':'b40','30m':'b30','20m':'b20','17m':'b17','15m':'b15','12m':'b12','10m':'b10','6m':'b6'};
const bandFreqs = {
  '80m':[3500,4000],'40m':[7000,7300],'30m':[10100,10150],'20m':[14000,14350],'17m':[18068,18168],
  ...
  try {
    ...
    if (hqRes.ok) {
      ...
      if (!hq.error) {
        ...
        // Radio blackout alert -- same xray field, classified against NOAA's R-scale
        updateBlackoutBanner(classifyRadioBlackout(xray), xray);
...
function rateBand(sfi, kIdx, band, tod) {
  ...
  const models = {
    ...
    '30m': { night:[75,95],  dawn:[75,95],  day:[95,120],  dusk:[75,95]  },
...
function updateBandConditions(sfi, kIdx) {
  ...
  const bandIds = ['80m','40m','30m','20m','17m','15m','12m','10m','6m'];
...
// ── Radio Blackout Alert ────────────────────────────────
// Classifies the same xray field already fetched by fetchSolarData() against
// NOAA's R-scale (radio blackout severity). No separate fetch/poll needed --
// this runs inline on the existing 5-minute solar refresh cycle. Independent
// dismiss/escalation state from the storm banner, and its own banner slot, so
// a solar flare and a thunderstorm can both be shown at the same time.
var blackoutDismissed = false;
var lastBlackoutLevel = 'NONE';

function classifyRadioBlackout(xrayStr) {
  // xrayStr looks like "C1.3", "M5.2", "X1.0". NOAA R-scale:
  //   A/B/C (any)  -> R0  (no alert)
  //   M1.0-M4.9    -> R1 (Minor)     -> CAUTION
  //   M5.0-M9.9    -> R2 (Moderate)  -> WARNING
  //   X1.0+        -> R3-R5 (Strong+)-> CRITICAL
  if (!xrayStr || xrayStr === '--') return 'NONE';
  const m = xrayStr.match(/^([A-Z])([\d.]+)/);
  if (!m) return 'NONE';
  const cls = m[1], mag = parseFloat(m[2]) || 0;
  if (cls === 'X') return 'CRITICAL';
  if (cls === 'M' && mag >= 5) return 'WARNING';
  if (cls === 'M') return 'CAUTION';
  return 'NONE';
}

function updateBlackoutBanner(level, xrayStr) {
  const banner = document.getElementById('blackoutBanner');
  const pill   = document.getElementById('blackoutPill');
  const icon   = document.getElementById('blackoutIcon');
  const title  = document.getElementById('blackoutTitle');
  const detail = document.getElementById('blackoutDetail');

  pill.className   = 'blackout-status-pill blackout-status-' + level;
  pill.textContent = (level === 'NONE') ? 'ALL CLEAR' : level;

  // Un-dismiss if severity escalates further (same pattern as storm banner)
  if (level !== 'NONE' && level !== lastBlackoutLevel) { blackoutDismissed = false; }
  lastBlackoutLevel = level;

  if (level !== 'NONE' && !blackoutDismissed) {
    const icons  = { CAUTION:'(!)', WARNING:'(!!)', CRITICAL:'[!!!]' };
    const titles = {
      CAUTION:  'R1 RADIO BLACKOUT -- MINOR HF DEGRADATION',
      WARNING:  'R2 RADIO BLACKOUT -- MODERATE HF DEGRADATION',
      CRITICAL: 'R3+ RADIO BLACKOUT -- SEVERE HF DEGRADATION'
    };
    banner.className  = 'blackout-banner visible ' + level;
    icon.textContent  = icons[level]  || '(!)';
    title.textContent = titles[level] || 'RADIO BLACKOUT';
    detail.innerHTML  = 'X-ray flare: ' + xrayStr +
      ' &nbsp;|&nbsp; HF degraded on sunlit path, low/mid bands hit hardest.<br>' +
      'No per-band frequency data available -- check band conditions directly.';
  } else if (level === 'NONE') {
    banner.className = 'blackout-banner';
  }
}

function dismissBlackout() {
  blackoutDismissed = true;
  document.getElementById('blackoutBanner').className = 'blackout-banner';
}
